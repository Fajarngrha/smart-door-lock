...
// -------------------- WiFi --------------------
char ssid[] = "NAMA_WIFI";
char wifiPass[] = "PASSWORD_WIFI";

// -------------------- RC522 pins (ESP32) --------------------
// SS/SDA -> GPIO5, RST -> GPIO22 (bisa diganti sesuai wiring)
constexpr uint8_t RFID_SS_PIN = 5;
constexpr uint8_t RFID_RST_PIN = 22;

// -------------------- Solenoid + door sensor --------------------
// LOCK_PIN ke gate MOSFET / input relay
constexpr uint8_t LOCK_PIN = 26;
// Sensor magnet/reed switch untuk status pintu
// Anggap LOW = tertutup, HIGH = terbuka (INPUT_PULLUP)
constexpr uint8_t DOOR_SENSOR_PIN = 27;
constexpr uint8_t GREEN_LED_PIN = 2;
constexpr uint8_t RED_LED_PIN = 4;
constexpr uint8_t BUZZER_PIN = 25;
// Sesuaikan bila relay module kamu active LOW (umum di pasaran)
...
void successAlarm() {
  pulsePin(GREEN_LED_PIN, 120);
  pulsePin(BUZZER_PIN, 120);
}

void deniedAlarm() {
  for (uint8_t i = 0; i < 3; i++) {
    pulsePin(RED_LED_PIN, 120);
    pulsePin(BUZZER_PIN, 120);
    delay(80);
  }
}
